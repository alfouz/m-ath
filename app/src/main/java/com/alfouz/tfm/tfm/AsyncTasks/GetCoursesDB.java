...
public class GetCoursesDB extends AsyncTask<Long, Void, List<Course>> {
    CallbackInterface callback;
    AppDatabase db;

    public GetCoursesDB(CallbackInterface callback, Context context) {
    ...
    protected List<Course> doInBackground(Long... integers) {
        ...
        List<CourseEntity> list = db.courseDao().getCourses();
        ...
        for(CourseEntity c : list){
            ...
            courses.add(new Course(c.getId(), c.getCreator(), c.getTitle(), listLesson, c.getLevel(), (listLessonEntity.size()!=0)?(score/listLessonEntity.size()):0, c.getDescription(), c.isPublic(), CourseType.getType(c.getType()), c.getIdRemote()));
        }
        return courses;
    }
